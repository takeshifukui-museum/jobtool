...
/** blockquote要素 / >記号見出し + 後続ul・p から抽出 */
const extractBlockquoteSections = (root) => {
  const sections = [];
  const seen = new Set(); // ラベル重複防止

  const pushSection = (label, valueEls, domTag) => {
    if (!label || label.length > 60 || seen.has(label)) return;
    const parts = [];
    for (const el of valueEls) {
      const text = normalizeNFKC(el.innerText || "").trim();
      if (text) parts.push(text);
    }
    const value = parts.join("\n").trim();
    if (value && !isUINoiseText(value)) {
      seen.add(label);
      sections.push({ label, value, domTag, snippet: value.slice(0, 120) });
    }
  };

  // Pattern A: <blockquote> 要素が見出しとして使われているケース
  for (const bq of root.querySelectorAll("blockquote")) {
    const rawLabel = normalizeNFKC(bq.innerText || "").replace(/\s+/g, " ").trim();
    // blockquote 内が長い場合はセクション本文の可能性が高い → スキップ
    if (!rawLabel || rawLabel.length > 60) continue;
    const label = rawLabel.replace(/^[>＞]\s*/, "").trim();
    const valueEls = [];
    let next = bq.nextElementSibling;
    while (next && next.tagName !== "BLOCKQUOTE" && !/^H[1-6]$/.test(next.tagName)) {
      if (["UL", "OL", "P", "DIV"].includes(next.tagName)) valueEls.push(next);
      next = next.nextElementSibling;
    }
    pushSection(label, valueEls, "blockquote");
  }

  // Pattern B: テキストが > または ＞ で始まる要素が見出しとして使われているケース
  for (const el of root.querySelectorAll("p, div, span, li")) {
    // 子要素が多い場合はコンテナの可能性 → スキップ
    if (el.children.length > 2) continue;
    const rawText = normalizeNFKC(el.innerText || "").replace(/\s+/g, " ").trim();
    if (!rawText.startsWith(">") && !rawText.startsWith("＞")) continue;
    const label = rawText.replace(/^[>＞]\s*/, "").trim();
    if (!label || label.length > 60) continue;
    const valueEls = [];
    let next = el.nextElementSibling;
    while (next && !/^H[1-6]$/.test(next.tagName) && next.tagName !== "BLOCKQUOTE") {
      const nextText = normalizeNFKC(next.innerText || "").replace(/\s+/g, " ").trim();
      // 次も > 見出しなら停止
      if (nextText.startsWith(">") || nextText.startsWith("＞")) break;
      if (["UL", "OL", "P", "DIV"].includes(next.tagName)) valueEls.push(next);
      next = next.nextElementSibling;
    }
    pushSection(label, valueEls, "blockquote-text");
  }

  return sections;
};

/** コンテナからセクション群を抽出（最も結果が多い方式を採用 + blockquoteをマージ） */
const extractSectionsFromContainer = (container) => {
  ...
  // blockquote / >記号セクションを追加抽出してマージ（既存ラベルは除外）
  const blockquote = extractBlockquoteSections(clone);
  const existingLabels = new Set(best.map((s) => s.label));
  const additional = blockquote.filter((s) => !existingLabels.has(s.label));
  const merged = [...best, ...additional];
  const finalMethod = additional.length > 0 ? `${method}+blockquote(${additional.length})` : method;

  return { sections: merged, method: finalMethod, noiseRemoved: noiseCount, cleanText: normalizeNFKC(clone.innerText || "").trim() };
};
